...
component = do
  window <- Web.window
  React.component "Editor" $ \ { paper, article: inputArticle, closeArticle, relogin, token: maybeToken } -> React.do
    ...
    let submit token = do
          ...
          Aff.launchAff_ $ liftEffect <<< either (setError <<< Just) (setSavedArticle <<< Just) =<<
            ((if isJust inputArticle
              ...
              else Lettera.createFallbackArticle) token article)
    pure $ fold
      ...
      , DOM.div
          ...
          , children:
              ...
              , InputField.inputField
                  ...
                  , name: "published"
                  , value: if publish
                           then formatLocalDateTime <$> (_.publishingTime =<< inputArticle)
                           else Nothing
                  , placeholder: ""
      ...
      , DOM.div
          ...
          , children:
              [ DOM.button
                  ...
                  , onClick: foldMap (capture_ <<< submit) maybeToken
                  , disabled: isNothing maybeToken
                  }
              , if isNothing maybeToken
                then DOM.text "Sessio vanhentunut, kopioi tallentamaton teksti leikepöydälle ja kirjaudu uudestaan."
                else mempty
              , DOM.button
      ...
      , foldMap (renderError relogin "Authentication error. Please copy changes you have to notepad and relogin.") error
      , guard success $
          DOM.div
