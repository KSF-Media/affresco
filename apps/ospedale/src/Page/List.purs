...
component = do
  ...
  React.component "List" $ \ props@{ newArticle, openArticle, setNavPaper, initialArticle } -> React.do
    ...
    useEffectOnce do
      flip foldMap initialArticle $ \uuid -> do
        Aff.launchAff_ $ Spinner.withSpinner setLoading do
          liftEffect <<< either
            (setError <<< Just)
            (setOpenedArticle <<< Just <<< Just) =<< Lettera.getSingleFallbackArticle uuid
      pure $ pure unit
    ...
    let loadArticles p = do
          ...
          setError Nothing
          Aff.launchAff_ do
            response <- Lettera.getFallbackArticles 0 50 p
            liftEffect $ case (\(Lettera.LetteraResponse l) -> l.body) response of
              Right a -> setArticles $ const $ Just a
              Left err -> setError $ Just $ show err
            pure unit
        paperSelect str = do
          ...
          flip foldMap maybePaper $ \p -> do
            setPaper $ Just p
            loadArticles p
        loadMore = flip foldMap paper $ \p -> do
          flip (maybe (pure unit)) articles $ \xs -> do
            let len = length xs
            Aff.launchAff_ do
              moreArticles <- Lettera.getFallbackArticles len 50 p
              liftEffect $ case (\(Lettera.LetteraResponse l) -> l.body) moreArticles of
                Left err -> setError $ Just $ show err
