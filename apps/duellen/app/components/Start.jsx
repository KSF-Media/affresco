  ...
  async componentDidMount() {
    ...
    try {
      ...
      const res = await fetch(
        backendURL +
          "get/all/quizzes/as/json/?lowerlim=" +
          this.state.lowerLim.toString() +
          "&upperlim=" +
          this.state.gap.toString()
      );
            ...
            <div>
              <div className="text-center m-auto col-12">
                ...
                <a href={"/Intro/" + this.state.weeklyQuiz.id}>
                  <Button variant="contained" className="start" fullWidth={true} color="primary">
                    Vecka {this.getweek(this.state.weeklyQuiz.publication_date)}
                  </Button>
                </a>
                ...
                {this.state.quizData.map((item) => (
                  <div key={item.id} className="btnOrange">
                    <a href={"/Intro/" + item.id}>
                      <Button variant="contained" className="start" color="primary" fullWidth={true}>
                        Vecka {this.getweek(item.publication_date)}
                      </Button>
                    </a>
                  </div>
                ))}
