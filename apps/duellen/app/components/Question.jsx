...
export default class Question extends React.Component {
  ...
  handleWrongRight(answer) {
    ...
    if (answer === this.checkIfCorrect()) {
      ...
      if (hintPoint === 1) {
        this.hideMessage();
        var notification = cogoToast.info("Den for tyvärr fel men nu var det dags för nästa", {
          toastContainerID: "1",
        });
  ...
  getNextHint() {
    if (this.state.hintPoint === 1) {
      ...
      const questionOptions = Object.getOwnPropertyNames(this.state.quizData.questions);
      const hintOptions = Object.getOwnPropertyNames(
        this.state.quizData.questions[questionOptions[this.state.progress - 1]].hints
      ).sort();
      for (var i = 0; i < 4; i++) {
        if (
          this.state.hint ===
          this.state.quizData.questions[questionOptions[this.state.progress - 1]].hints[hintOptions[i]]
        ) {
          this.setState({
            ...
            showHintsText: true,
            pastQuestionHints: [
              ...this.state.pastQuestionHints,
              [
                this.state.hintPoint + " poängs ledtråden",
                this.state.quizData.questions[questionOptions[this.state.progress - 1]].hints[hintOptions[i]],
              ],
            ],
          });
  ...
  render() {
    ...
    if (logged_in === false) {
      ...
      if (displayResult === true) {
        ...
        return (
          <MiddleScreen
            players={this.state.quizData.players}
            progress={this.state.progress - 1}
...
*/}
            ...
            <div className="row">
              ...
              {this.state.pastQuestionHints.length > 0 && (
                <div className="col-6">
                  <button onClick={this.showHints.bind(this)} className="show-as-p">
                    Se tidigare ledtrådar <span className="turn">&#9660;</span>
                  </button>
                </div>
              )}
            </div>
            <div className="row">
              <div className="col text-center">
                <h2>
                  {this.state.category}: {this.state.question}
                </h2>
              </div>
            </div>
            <PastHints
              hints={this.state.pastQuestionHints}
              show={this.state.showPastHints}
              lasthint={this.state.hintPoint}
            ></PastHints>
            <div className="row">
              <div className="col text-center">
            ...
            <div className="row">
              <div className="col-12 mt-3">
                <input
                  autoComplete="off"
                  id="input_text"
                  className="w-100 p-1 text-dark bg-black"
                  type="text"
                  value={this.state.userInput}
                  onChange={this.inputChange.bind(this)}
                  placeholder="Skriv in svaret här..."
                ></input>
              </div>
            </div>
            <div className="row">
              <div className="col">
                ...
                <div id="output_options" className="d-flex flex-column">
                  <WikiLink search={this.state.searchText} onClick={this.handleClick.bind(this)}></WikiLink>
                </div>
              </div>
            </div>
            ...
            <div className="row">
              <div className="col mt-3 text-right">
                <button onClick={this.skipHint.bind(this)} className="skipButton questionBtn">
                  Nästa ledtråd
                </button>
              </div>
              <div className="col mt-3 text-left">
                <button onClick={this.skipAnswer.bind(this)} className="skipButton questionBtn">
                  Nästa fråga
                </button>
              </div>
            </div>
