...
export default class App extends React.Component {
  ...
  onAreaSelection(areaId) {
    this.stopFuse();
    if (!areaId) {
      // if no area was provided (e.g. on start or reset)
      return this.onAreaSelection("MAA");
    }
    return getArea(areaId).then((areaResponse) => {
  ...
  getCountryData() {
    return getArea("MAA").then((countryResponse) => {
      ...
      let s = {
        seats: countryResponse.nominators.map((nominator) => {
          return {
      ...
      case "/compact":
        return (
          <div className={isMobile ? "mobile ksf-elections compact" : "ksf-elections compact"}>
            <AreaInfo areaResponse={this.state.selectedAreaResponse} />
            <Chart areaResponse={this.state.selectedAreaResponse} />
          </div>
        );
      default:
          ...
          case "PARLIAMENT":
            return (
              <div className={isMobile ? "mobile ksf-elections" : "ksf-elections"}>
                <Status
                  percentage={
                    this.state.countryResponse ? this.state.countryResponse.area.info.calculationStatus : null
                  }
                />
                <Timestamp timestamp={this.state.countryResponse ? this.state.countryResponse.timestamp : null} />
                <Parliament seats={this.state.seats} />
                <AreaPicker
                  onSelection={(area) => {
                    if (area === null) {
                ...
                <AreaInfo areaResponse={this.state.selectedAreaResponse} />
                <Chart areaResponse={this.state.selectedAreaResponse} />
                <Table areaResponse={this.state.selectedAreaResponse} />
              </div>
            );
          case "EU":
            return (
