...
class App extends React.Component {
  ...
  render() {
        ...
        <nav className="pod-nav">
          ...
          <ul className="nav nav-lg">
            {this.state.podcasts.map((name, key) => {
              ...
              return (
                <li className={buttonClass} onClick={() => this.selectPodcast(name)} key={key}>
                  {name}
                </li>
              );
            })}
    ...
    // JSX: List of tracks
    const trackList = sortedTracks.map((t) => {
    ...
    return (
      <div className="pod-app">
        {podcastNav}
        {spinner}
        <div className="pod-tracklist">{trackList}</div>
      </div>
    );
  }
}
