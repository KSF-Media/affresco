...
renewSubscription :: Component Types.RenewSubscription
renewSubscription = do
  component "RenewSubscription" \props -> React.do
    ...
    useEffectOnce $ do
      setState _ { poller { onComplete = liftEffect do
                               setState $ \s -> s { form = case s.poller.orderState of
                                                       Right OrderCompleted -> AsyncWrapper.Ready
                                                       _ -> AsyncWrapper.Error ""
                                                  }
                               case state.poller.orderState of
                                 Right OrderCompleted -> props.onSuccess
                                 _ -> props.onError User.SomethingWentWrong
                          }
                 }
      pure (pure unit)
...
render props state setState resetForm =
  DOM.div
    { className: "clearfix renew-subscription--container"
    , children:
  ...
  where
    ...
    renderError err = WrapperElements.errorWrapper resetForm $ case state.poller.orderState of
    ...
    renderLoading =
      React.fragment $
        ...
        (case state.netsUrl of
            Just { paymentTerminalUrl } -> case state.poller.orderState of
              Right OrderCreated ->
                [ DOM.div
  ...
  DOM.div
    ...
    , children:
        [ Grid.row_
            ...
            , DOM.div
                { className: "col-1 flex renew-subscription--close-icon"
  ...
  Aff.launchAff_ $ do
    ...
    case eitherNetsUrl of
      ...
      Right { nets, order } -> liftEffect do
        Poller.start state.poller setPoller order.number
        setState _ { form = AsyncWrapper.Loading form
                   , netsUrl = nets
                   }
  where
    setPoller = \f -> setState $ \s -> s { poller = f s.poller }
